---
ref: FSM__Finite-State-Machine-untuk-E2E-Testing
judul: 'FSM: Finite State Machine untuk E2E {end-to-end} Testing'
title: 'FSM: Finite State Machine untuk E2E {end-to-end} Testing'
description: ''
tags: [idea]
category: Idea
category_url: idea
...
Pengalaman saya menggunakan `Playwright` untuk E2E {end-to-end} Testing, terhadap banyak Form:

- function `page.waitForTimeout()` cukup bagus untuk beberapa situasi
- tetapi jika ada bagian `FrontEnd` yang melakukan beberapa `fetch` secara _cascade_ == cerita yang berbeda
- saya memerlukan suatu mekanisme yang konsisten
- saya harus membuat "suatu Pola" yang _robust_ untuk E2E {end-to-end} Testing

## FSM: konsep pada file `JavaScript`

```js
// contoh implementasi — bisa disesuaikan dengan framework apapun
...
Kita bisa membuat implementasi konsep FSM ini menggunakan `JavaScript Framework/Library` yang biasa digunakan. Untuk saat ini, saya pilih `VUE` seperti berikut:
...
async function showData() {
    ...
    if (!response.data) {
        ...
        dataRefs.page_Message = `Data dengan code '${code}', tidak ditemukan`
...
async function saveData(skipConfirmation = false) {
    ...
    // - Loading tetap true
    // - page_Kondisi diubah menjadi 'submit_success'
    Global_Data_APP.setProcessing(dataRefs, true, 'submit_success')

    // - di sini, Loading diubah menjadi false
    Global_Data_APP.messageInfo(dataRefs, 'Data berhasil disimpan', () => sdr_GoBack())
}
